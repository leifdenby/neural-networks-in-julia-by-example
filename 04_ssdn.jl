...
begin
	function pad_offset(x, offset; dim=2)
		"add `offset` number of zeros"
		pad_zeros(x, (offset,0), dims=[dim])
	end
	N = 256
	conv_size = 3
	a = randn(Float32, (N, N, 1, 1))
	n_offset = conv_size ÷ 2
	pad_offset(a, n_offset)
end
...
begin
	...
	@assert rotate_hw(a, 90) == rotate_hw(a, -270)
end

# ╔═╡ 5d19650f-f76e-4a4d-a9da-71ded5804bff
begin
	#download("http://labs.sogeti.com/wp-content/uploads/2014/03/cloud.jpg", "cloud.jpg")
	img_orig = load("cloud.jpg")
	img = imresize(img_orig, (256, 256))
end

# ╔═╡ 9ab28319-f9f2-4aa7-b268-22bc85736218
begin
	...
	function apply_model_to_single_image(img, model)
		arr = img_to_arr(img)
	...
	# apply_model_to_single_image(img, HalfPlaneMaxPool2D(5, 1))
	# apply_model_to_single_image(img, HalfPlaneConv2D((3, 3), 3=>3, identity))
	apply_model_to_single_image(img, x -> rotate_hw(x, -90))
end

# ╔═╡ 76b923ee-1eca-4e8c-85ca-3b6594f36dd9
...
begin
	function make_ssdn(nc_in, n_levels, n_out_features)
		...
		model = Chain(
			HalfPlaneConv2D((w,w), nc_in => nc_hd, LR),
			lower_level,
			HalfPlaneConv2D((w,w), 2*nc_hd => n_out_features, LR)
		)
		return model
	end
