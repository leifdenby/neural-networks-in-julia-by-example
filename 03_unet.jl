...
begin
	function train_model(model, img, target, n_epochs::Int; use_gpu=false)
		function loss(x, y)
	...
	function train_and_plot_model(model, img, cloud_mask; n_epochs=20, use_gpu=false)
		target = cloud_mask |> mask_to_arr
		losses = train_model(model, img, target, n_epochs, use_gpu=use_gpu)
...
# ╔═╡ 336249b9-eea1-4d32-a3c6-9fa37716a86c
md"""
TODO: what is a convolution-transpose?
"""

# ╔═╡ 8b3e01be-5dab-48e5-8ee7-7704909caae7
md"""
## Builing a skip-connection

TODO: what is a skip-connection and why do we need it?
"""

# ╔═╡ 25ffdbd7-1d22-4086-9a3b-5d146d3f3cff
begin
	# Let's first define the branch of the network in which
	# the input is downscaled (using max-pooling) and then up-scaled
	...
	m_du_with_skip = Chain(
		...
		# and then halve the number of channels again using a 1x1 convolution
		Conv((1,1), 6=>3)
	)
end
...
begin
	...
	@show size.([img_small, img_mp, img_ct, img_sk])
end

# ╔═╡ 46fa62d2-2812-47bb-b304-f9c8502f38d7
begin
	plot(layout=4, link=:all, titlefontsize=8)
	images = [
		("input", img_small), 
		("max-pool", img_mp), 
		("down-scaling\n+ up-scaling", img_ct), 
	...
	for (i, (_label, _img)) in enumerate(images)
		plot!(_img, subplot=i, title=_label * " " * string(size(_img)))
	end
	plot!()  # needed when using Pluto.jl, otherwise the plot wont show
end

# ╔═╡ 84ba6010-c941-4673-9ccb-c0c0a602e854
with_terminal() do
	x_ = img_small |> img_to_arr |> single_to_batch
	@show size(x_)
	for op in m_downscale_upscale
		x_ = op(x_)
		@show op size(x_)
	end
end
...
# ╔═╡ 89567b33-8041-4c3e-817c-39f10a45bdcd
m_unet1 = one_layer_unet()

# ╔═╡ aaab57ed-53d6-4338-9f3f-bea4ee1b4c64
begin
	x_1 = randn(Float16, (9, 9, 3, 1))
	y_1 = m_unet1(x_1)
	size.([x_1, y_1])
end

# ╔═╡ 18b07895-9c91-4493-bc20-9d37a9146075
train_and_plot_model(m_unet1, img, Float32.(cloud_mask), n_epochs=40, use_gpu=true)
...
begin
	function make_unet(nc_in, n_out_features, n_levels; nc_hd=4, w=3, mp=2, activation=relu)
		"""
		nc_hd: number of hidden channels
		w: width of convolution
		mp: max-pooling and upscaling (convolution-transpose) size
		"""
		lower_level = undef
		...
		# we construct the UNet starting from the lowest (coarsest) level
		for n in 1:n_levels
			# every level in the UNet starts with a convolution and max-pooling
			layers = [
				# offset convolution with padding
				_Conv((w,w), nc_hd => nc_hd, activation, pad=pad),
				# max-pool coarsening
				_MaxPool((mp,mp))
			]
			
			if n > 1
				# if we're above the lawest level we will include a skip-
				# connection to include the lower level
				append!(layers, [
					...
					_Conv((w,w), nc_hd*3 => nc_hd, activation, pad=pad)
				])
			end

			# all UNet levels end with a convolution and then up-scaling
			# using a conv-transpose
			append!(layers,
				[
				# offset convolution with padding
				_Conv((w,w), nc_hd => nc_hd, activation),
				# conv-tranpose up, doubling number of channels
				ConvTranspose((mp,mp), nc_hd => 2*nc_hd, stride=mp)
			])
		...
		model = Chain(
			_Conv((w,w), nc_in => nc_hd, activation),
			lower_level,
			_Conv((w,w), 2*nc_hd => n_out_features, activation)
		)
		return model
	end
end

# ╔═╡ 3865143c-d4c9-4513-850d-d34c5ed86ea3
begin
	function make_unet2(nc_in, n_out_features, n_levels; nc_hd=4, w=3, mp=2, activation=relu)
		"""
		nc_hd: number of hidden channels
		w: width of convolution
		mp: max-pooling and upscaling (convolution-transpose) size
		...
		# we construct the UNet starting from the lowest (coarsest) level
		for n in 1:n_levels
			# number of features in the output of this level
			nf_level_out = nc_hd * 2 ^ (n_levels - n)
			if n == n_levels
				nf_level_in = 1
			else
				nf_level_in = Int(nf_level_out / 2)
			end

			@show nf_level_in nf_level_out
			
			# every level in the UNet starts with a convolution
			level_layers = [
				# convolution with padding
				_Conv((w,w), nf_level_in => nf_level_out, activation, pad=pad),
				# original UNet has two convolutions per level
				# _Conv((w,w), nf_level => nf_level, activation, pad=pad),
			]
			...
			if n > 1
				...
				lower_level_branch = Chain(
					# max-pool coarsening
					_MaxPool((mp,mp)),
					levels[n-1],
					# conv-tranpose up, doubling number of channels
					ConvTranspose((mp,mp), nf_level_in => nf_level_out, stride=mp),
					
				)

				x_ = randn(Float32, (32, 32, nf_level_out, 1))
				@show size.([x_, lower_level_branch(x_)])
				skip = SkipConnection(lower_level_branch, (mx, x) -> cat(mx, x, dims=3))
				
				@show size.([skip(x_), x_])
				#append!(level_layers, [skip])
				
				#append!(level_layers, [
					# skip-connetion with concatenation across lower level
				#	SkipConnection(lower_level_branch, (mx, x) -> cat(mx, x, dims=3))
					# conv-transpose up in lower level doubled number channels
					# so with concat that is (2+1)*nc_hd. Conv with padding
					# to return to nc_hd channels
					# _Conv((w,w), nf_level_out*2 => nf_level_out, activation, pad=pad)
				#])
			end
			append!(levels, level_layers)
		end
		
		model = Chain(
			levels[n_levels],
			_Conv((1,1), nc_hd => n_out_features)
		)
		return model
	end
end

# ╔═╡ 4b7d81d3-265b-4f91-a0a7-c9ca64206e93
with_terminal() do
	m_unet_general_1layer = make_unet2(3, 1, 2; nc_hd=64)
end

# ╔═╡ e15cb776-dc9e-475d-886c-dc11db07b54b
m_unet1

# ╔═╡ 00c2d050-cb2f-4b2e-bf52-26bca73fce55
begin
	x = img |> img_to_arr |> single_to_batch
	m_unet_general_1layer = make_unet2(3, 1, 1; nc_hd=3)
	size.([x, m_unet_general_1layer(x)])
end

# ╔═╡ e8d81526-1dc8-4ecf-817f-620a7583fac0
begin
#with_terminal() do
	m_unet_layer2 = make_unet(3, 1, 1)
	train_and_plot_model(m_unet_layer2, img, Float32.(cloud_mask), n_epochs=10, use_gpu=false)
end

# ╔═╡ ded675f2-a233-4a77-b2f5-985e2681e2b5
...
git-tree-sha1 = "e25f12e4266518b3f551e144ea444a4f425c09ca"
